...
def questionInfo(app):
    ...
    #eat more suggestion
    if app.noFoodButton == True or app.oneButton == True:
        suggestionsList.append('''Pro tip: carry snacks with you to 
        munch on throughout the day :)''')
    #drink more suggestion
    if app.noButtonThree == True:
        suggestionsList.append('''Pro tip: carry a waterbottle with you and mark
        down how much you want to drink for each hour :)''')
    #sleeping moresuggestion
    if app.noButtonFour == True:
        suggestionsList.append('''Pro tip: Put your phone away 30 minutes
        before you sleep! Try meditating, reading, or doing another relaxing
        activity before you sleep :)''')
    #exercising more suggestion
    if app.noButtonFive == True:
        suggestionsList.append('''Pro tip: exercising can be fun! Get your 
        body moving by blasting your favorite movie and dancing like no one is 
        watching :)''')
    #spending time with people suggestion
    if app.noButtonSix == True:
        suggestionsList.append('''Pro tip: 
        Hugging can relieve stress and give a sense of calmness. Try giving
        someone a hug today :)''')
    #going outside suggestion
    if app.noButtonSeven == True:
        suggestionsList.append('''Pro tip: is there a friend you've been wanting
        to catch up with? Call them, and ask if they'd like to go on a walk :)''')
    #meditating suggestion
    if app.noButtonEight == True:
        suggestionsList.append('''Pro tip: Sit down for 5 minutes and reflect on
        3 things that you are grateful for today :)''')

    return questionList, answerButtons, suggestionsList
